 ...
 * License along with FFmpeg; if not, write to the Free Softwareav_drm_register
 ...
 * add by gs for drm
 * we will get
 * server addrs drm_key_version drm_key_version_next
 * from metadata
 * then get cur key from server
 * and then go on or failed
**/
    void* drm_thiz;
    char drm_key[20];
    char* drm_version;
    char drm_key_next[20];
    char* drm_version_next;
    int drm_flags;
    int drm_length_size;
} AVFormatContext;
...
/**
 * add by gs for drm
 * set drm callback funtion
 **/
int av_drm_register(int (*get_key_cb1)(void* thiz, char* version, int curflag)
                   /*,int (*get_new_key_cb2)(void* thiz,int curflag)*/
                    ,int (*dec_cb3)(void* thiz, uint8_t* buf, int size)
                   ,int (*enc_cb4)(void* thiz, uint8_t* buf, int size)
                     );
/**
 * end
**/
...
/**
 * ksyplayer custom API
 */

typedef struct KSYFormatSegmentConcatContext {
    /**
     ...
     */
    int count;
} KSYFormatSegmentConcatContext;

typedef struct KSYFormatSegmentContext {
    /**
     * Segment position (0-based) in stream
     *
     * set by demux
...
} KSYFormatSegmentContext;

/**
 * Resolve segment count
 *
 * @param data      KSYFormatSegmentConcatContext pointer
 * @param data_size size of data;
 * @return 0 if OK, AVERROR_xxx otherwise
 */
#define KSYAVF_CM_RESOLVE_SEGMENT_CONCAT 0x10000

/**
 * Resolve segment url from concatdec
 *
 * @param data      KSYFormatSegmentContext pointer
 * @param data_size size of data;
 * @return 0 if OK, AVERROR_xxx otherwise
 */
#define KSYAVF_CM_RESOLVE_SEGMENT 0x10001
 ...
 * @return 0 if OK, AVERROR_xxx otherwise
 */
#define KSYAVF_CM_RESOLVE_SEGMENT_OFFLINE 0x10002
